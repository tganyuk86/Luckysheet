...
export function countfunc() {
    ...
    if (isFinite(max) || isFinite(min)) {
        ret += "<span>"+locale_formula.sum+":" + Math.round(sum*100)/100 + "</span>";
        ret += "<span>"+locale_formula.average+":" + ( Math.round(sum / count * 100) / 100) + "</span>";
    }

    if (isFinite(max)) {
        ret += "<span>"+locale_formula.max+":" + ( Math.round(max*100)/100) + "</span>";
    }

    if (isFinite(min)) {
        ret += "<span>"+locale_formula.min+":" + (Math.round(min*100)/100) + "</span>";
    }
